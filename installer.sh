...
#    Copyright (c) 2020, Darijo Raca, MaÃ«lle Manifacier, John O'Sullivan, Jason Quinlan
...
#
#John Oâ€™Sullivan, D. Raca, and Jason J. Quinlan.  Demo Paper: godash 2.0 - The Next Evolution of HAS Evaluation. 21st IEEE International Symposium On A World Of Wireless, Mobile And Multimedia Networks (IEEE WoWMoM 2020), Cork, Ireland. August 31 to September 03, 2020

# version 1.0.20 - 01-08-2020
# few small updates to structure of the script
# update to godashbed v2.0.2 - minor fixes for quic
    ...
    # remove dependencies for goDASH(bed)
        ...
        if ! [ -n "$varname" ]; then
            ...
            then
                ...
                sudo sed -i 's|10.0.0.1   www.godashbed.org||g' /etc/hosts
...
then
    ...
    echo ""
    sudo apt install net-tools build-essential git python3-pip sed unzip -y
    sudo pip3 install pandas
    sudo pip3 install numpy
    sudo pip3 install matplotlib
    
    # install the ASGI Server - Hypercorn/Quart - with HTTP/3 (QUIC) support
    sudo pip3 install hypercorn[h3]==0.10.2
    sudo pip3 install Quart
    sudo python3 -m pip install -U quart-trio
    ...
    cd itu-p1203/
    sudo pip3 install .
    sudo python3 -m itu_p1203 examples/mode0.json
    cd ..
    
    # create sim link to p1203-standalone, as xterm does not read from $HOME/.local/bin
    sudo ln -s $HOME/.local/bin/p1203-standalone /usr/local/bin/p1203-standalone
    ...
    git clone https://github.com/reza92farahani/godash.git
    # move into the repo
    cd godash
